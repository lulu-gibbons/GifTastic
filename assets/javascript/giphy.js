  ...
  $.ajax({
    ...
    //  console.log(response);
      ...
      for (var i = 0; i < results.length; i++) { //loops through the images in the array and grabs information needed and stores that info in variables
        ...
        var rating = results[i].rating; //grabs the rating from each gif
        ...
        image.attr("src", still); //adding the still image to each image tag
        ...
        image.addClass("gif-image img-responsive img-thumbnail");
...
$("#add-input").on("click", function(event) {
  ...
  var newUserInput = $("#user-input").val().trim(); // grabs the user's input
